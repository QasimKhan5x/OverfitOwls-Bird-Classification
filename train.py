...
def load_data(traindir, valdir, args):
    ...
    print("Crop Sizes:", val_crop_size, val_resize_size, train_crop_size)
    ...
    if args.cache_dataset and os.path.exists(cache_path):
        ...
        dataset_test = torchvision.datasets.ImageFolder(
            valdir, preprocessing, target_transform=utils.class_transform
        )
        if args.cache_dataset:
...
def main(args):
    ...
    train_acc1_list, valid_acc1_list = [], []
    ...
    # train_dir = os.path.join(args.data_path, "train_images")
    train_dir = os.path.join(args.data_path, "combined_images")
    val_dir = os.path.join(args.data_path, "val_images")
    ...
    start_time = time.time()
    best_train_acc1, best_valid_acc1 = 0.0, 0.0
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            # Save the best checkpoint based on acc@1
            ...
            valid_acc1_list.append(valid_acc1)
    ...
    # Accuracy curves
    plt.subplot(1, 2, 2)
    plt.plot(epochs, train_acc1_list, label="Train Acc@1")
    plt.plot(epochs, valid_acc1_list, label="Valid Acc@1")
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--data-path",
        default="/gpfs/workdir/yutaoc/bird/bird_dataset",
        type=str,
        help="dataset path",
    )
    parser.add_argument(
        "--model", default="efficientnet_v2_s", type=str, help="model name"
    )
    parser.add_argument(
        "--device",
        default="cuda",
